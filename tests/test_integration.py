        ...
        try:
            ...
            for i in range(5):
                engine.record_result(
                    session_id=f"sess-{i}", outcome="success",
                    prompt=f"fix login bug #{i}", approach="Targeted Fix",
                    model="sonnet", files_changed=["src/auth.py", "tests/test_auth.py"],
                    test_results={"total": 8, "passed": 8, "failed": 0},
                    duration_seconds=30.0, token_count=5000,
                )
            for i in range(3):
                engine.record_result(
                    session_id=f"bad-{i}", outcome="failure",
                    prompt=f"fix login bug #{i}", approach="Aggressive Rewrite",
                    model="opus",
                    files_changed=[f"src/module{i}.py" for i in range(12)],
                    test_results={"total": 10, "passed": 4, "failed": 6},
                    duration_seconds=120.0, token_count=20000,
                )
            ...
            assert suggestion["classification_confidence"] > 0.8
...
class TestSQLiteStore:
    """Tests for the production SQLite backend."""
    ...
    def test_upsert_and_get_symbol(self, tmp_path: Path):
        store = SQLiteStore(tmp_path / "test.db")
        sym = Symbol(id="test-sym-1", name="hello", kind=SymbolKind.FUNCTION,
    ...
    def test_upsert_file(self, tmp_path: Path):
        ...
        store = SQLiteStore(tmp_path / "test.db")
        fs = FileSummary(path="src/main.py", language=".py", line_count=50, imports=["os", "sys"])
    ...
    def test_search_symbols(self, tmp_path: Path):
        ...
        for name, kind in [("auth_user", SymbolKind.FUNCTION), ("login", SymbolKind.FUNCTION), ("billing_total", SymbolKind.FUNCTION)]:
            store.upsert_symbol(Symbol(id=f"id-{name}", name=name, kind=kind, file_path=f"src/{name}.py", line_start=1, line_end=5))
    ...
    def test_record_and_get_session(self, tmp_path: Path):
        store = SQLiteStore(tmp_path / "test.db")
        session = SessionRecord(session_id="sess-sqlite-1", repository="test", prompt="Fix bug",
    ...
    def test_approach_efficacy_tracking(self, tmp_path: Path):
        store = SQLiteStore(tmp_path / "test.db")
        ae = store.update_approach_efficacy(problem_type="bug_fix", approach="Targeted", was_successful=True, model="sonnet", duration_seconds=30.0, token_count=5000)
        assert ae.success_rate == 1.0
        ae = store.update_approach_efficacy(problem_type="bug_fix", approach="Targeted", was_successful=False, model="opus", duration_seconds=60.0, token_count=10000)
    ...
    def test_persistence_across_instances(self, tmp_path: Path):
        ...
        store1.upsert_symbol(Symbol(id="persist-sym", name="persist_me", kind=SymbolKind.FUNCTION, file_path="test.py", line_start=1, line_end=5))
        store1.close()
    ...
    def test_build_context(self, tmp_path: Path):
        store = SQLiteStore(tmp_path / "test.db")
        store.upsert_symbol(Symbol(id="ctx-sym", name="authenticate", kind=SymbolKind.FUNCTION, file_path="src/auth.py", line_start=1, line_end=10, signature="def authenticate(token: str) -> User:", docstring="Authenticate a user from a JWT token."))
        store.learn_pattern(pattern_type="error_handling", description="Use 401 for auth failures", examples=["src/auth.py"], source_session_id="sess-1")
    ...
    def test_learn_patterns(self, tmp_path: Path):
        store = SQLiteStore(tmp_path / "test.db")
        store.learn_pattern(pattern_type="testing", description="Write tests alongside code", examples=["a.py", "b.py"], source_session_id="sess-1")
...
class TestLearningEngineProtocol:
    """LearningEngine works with both JSON store and SQLite store."""

    def test_engine_with_json_store(self, tmp_path: Path):
        store = KnowledgeGraphStore(tmp_path / "test_json")
        engine = LearningEngine(store)
        session = SessionRecord(session_id="sess-protocol", repository="test", prompt="fix bug", outcome=AgentOutcome.SUCCESS)
    ...
    def test_engine_with_sqlite_store(self, tmp_path: Path):
        store = SQLiteStore(tmp_path / "test.db")
        engine = LearningEngine(store)
        session = SessionRecord(session_id="sess-sql-protocol", repository="test", prompt="fix error", outcome=AgentOutcome.SUCCESS)
        result = engine.ingest_session(session)
        assert result["status"] == "ingested"
        assert result["problem_type"] == "bug_fix"
        store.close()
...
class TestOpenInspectBridge:
    """Bridge creates and reuses engines properly."""
