...
class SQLiteStore:
    ...
    def set_file_hash(self, file_path: str, hash_value: str) -> None:
        conn = self._get_conn()
        conn.execute("INSERT OR REPLACE INTO file_hashes VALUES (?, ?, ?)", (file_path, hash_value, self._now()))
        conn.commit()
    ...
    def upsert_symbol(self, symbol: Symbol) -> None:
        conn = self._get_conn()
        conn.execute("""INSERT OR REPLACE INTO symbols (id, name, kind, file_path, line_start, line_end, signature, docstring, complexity_score, last_modified, first_seen) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (symbol.id, symbol.name, symbol.kind.value, symbol.file_path, symbol.line_start, symbol.line_end, symbol.signature, symbol.docstring, symbol.complexity_score, symbol.last_modified.isoformat(), symbol.first_seen.isoformat()))
    ...
    def upsert_file(self, file_summary: FileSummary) -> None:
        conn = self._get_conn()
        conn.execute("""INSERT OR REPLACE INTO files (path, language, summary, line_count, last_indexed) VALUES (?, ?, ?, ?, ?)""",
            (file_summary.path, file_summary.language, file_summary.summary, file_summary.line_count, file_summary.last_indexed.isoformat()))
    ...
    def get_file(self, path: str) -> FileSummary | None:
        ...
        if not row:
            return None
        imports = conn.execute("SELECT import_statement FROM file_imports WHERE file_path = ?", (path,)).fetchall()
        return FileSummary(path=row["path"], language=row["language"], summary=row["summary"] or "", line_count=row["line_count"], last_indexed=datetime.fromisoformat(row["last_indexed"]), imports=[i[0] for i in imports])
    ...
    def search_symbols(self, query: str, kind: str | None = None, file_path: str | None = None) -> list[Symbol]:
        conn = self._get_conn()
    ...
    def get_symbol_dependencies(self, symbol_id: str) -> list[Symbol]:
        conn = self._get_conn()
        rows = conn.execute("""SELECT s.* FROM symbols s JOIN symbol_deps d ON s.id = d.dependency_id WHERE d.symbol_id = ?""", (symbol_id,)).fetchall()
        return [self._row_to_symbol(r) for r in rows]

    def get_symbol_dependents(self, symbol_id: str) -> list[Symbol]:
        conn = self._get_conn()
        rows = conn.execute("""SELECT s.* FROM symbols s JOIN symbol_deps d ON s.id = d.symbol_id WHERE d.dependency_id = ?""", (symbol_id,)).fetchall()
        return [self._row_to_symbol(r) for r in rows]
    ...
    def get_impact_radius(self, symbol_id: str, depth: int = 2) -> list[Symbol]:
        ...
        for _ in range(depth):
            ...
            rows = conn.execute(f"SELECT s.id FROM symbols s JOIN symbol_deps d ON s.id = d.symbol_id WHERE d.dependency_id IN ({placeholders})", list(current)).fetchall()
            current = {r[0] for r in rows} - affected
            affected.update(current)
        if not affected:
            return []
        placeholders = ",".join("?" * len(affected))
        rows = conn.execute(f"SELECT * FROM symbols WHERE id IN ({placeholders})", list(affected)).fetchall()
        return [self._row_to_symbol(r) for r in rows]

    def _row_to_symbol(self, row: sqlite3.Row) -> Symbol:
        conn = self._get_conn()
        dep_rows = conn.execute("SELECT dependency_id FROM symbol_deps WHERE symbol_id = ?", (row["id"],)).fetchall()
        return Symbol(id=row["id"], name=row["name"], kind=SymbolKind(row["kind"]), file_path=row["file_path"], line_start=row["line_start"], line_end=row["line_end"], signature=row["signature"], docstring=row["docstring"] or "", dependencies=[r[0] for r in dep_rows], complexity_score=row["complexity_score"], last_modified=datetime.fromisoformat(row["last_modified"]), first_seen=datetime.fromisoformat(row["first_seen"]))

    def record_session(self, session: SessionRecord) -> None:
        conn = self._get_conn()
        conn.execute("""INSERT OR REPLACE INTO sessions VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (session.session_id, session.repository, session.prompt, session.approach, session.model, session.outcome.value, session.pr_url, session.duration_seconds, session.token_count, session.created_at.isoformat(), session.completed_at.isoformat() if session.completed_at else None))
        ...
        conn.execute("INSERT OR REPLACE INTO session_test_results VALUES (?, ?)", (session.session_id, json.dumps(session.test_results)))
    ...
    def get_session(self, session_id: str) -> SessionRecord | None:
        ...
        if not row:
            return None
        files = conn.execute("SELECT file_path FROM session_files WHERE session_id = ?", (session_id,)).fetchall()
        test_row = conn.execute("SELECT results_json FROM session_test_results WHERE session_id = ?", (session_id,)).fetchone()
        comments = conn.execute("SELECT comment FROM session_review_comments WHERE session_id = ?", (session_id,)).fetchall()
        return SessionRecord(session_id=row["session_id"], repository=row["repository"], prompt=row["prompt"], approach=row["approach"], model=row["model"], outcome=AgentOutcome(row["outcome"]), pr_url=row["pr_url"], files_changed=[f[0] for f in files], test_results=json.loads(test_row[0]) if test_row else {}, review_comments=[c[0] for c in comments], duration_seconds=row["duration_seconds"], token_count=row["token_count"], created_at=datetime.fromisoformat(row["created_at"]), completed_at=datetime.fromisoformat(row["completed_at"]) if row["completed_at"] else None)
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        completed = conn.execute("SELECT COUNT(*) FROM sessions WHERE outcome != ?", (AgentOutcome.IN_PROGRESS.value,)).fetchone()[0]
        successes = conn.execute("SELECT COUNT(*) FROM sessions WHERE outcome = ?", (AgentOutcome.SUCCESS.value,)).fetchone()[0]
        return {"total_symbols": total_symbols, "total_files": total_files, "total_patterns": total_patterns, "total_sessions": total_sessions, "completed_sessions": completed, "successful_sessions": successes, "overall_success_rate": successes / completed if completed > 0 else 0.0, "total_approaches_tracked": conn.execute("SELECT COUNT(*) FROM approaches").fetchone()[0], "graph_nodes": conn.execute("SELECT COUNT(*) FROM symbols").fetchone()[0], "graph_edges": conn.execute("SELECT COUNT(*) FROM symbol_deps").fetchone()[0]}
    ...
    def update_approach_efficacy(self, problem_type: str, approach: str, was_successful: bool, model: str, duration_seconds: float, token_count: int) -> ApproachEfficacy:
        ...
        if existing:
            ...
            # Compute best model from session data (no schema migration needed)
            best_model_row = conn.execute(
                "SELECT model, COUNT(*) as cnt FROM sessions WHERE outcome = ? AND approach = ? GROUP BY model ORDER BY cnt DESC LIMIT 1",
                (AgentOutcome.SUCCESS.value, approach),
            ).fetchone()
            best_model = best_model_row["model"] if best_model_row else model
            conn.execute(
            ...
            conn.commit()
            return ApproachEfficacy(problem_type=problem_type, approach=approach, total_attempts=new_total, successes=new_successes, avg_duration_seconds=new_avg_dur, avg_tokens=new_avg_tok, best_model=best_model)
        else:
            conn.execute(
            ...
            conn.commit()
            return ApproachEfficacy(problem_type=problem_type, approach=approach, total_attempts=1, successes=1 if was_successful else 0, avg_duration_seconds=duration_seconds, avg_tokens=token_count, best_model=model)
    ...
    def get_best_approaches(self, problem_type: str | None = None, min_attempts: int = 3) -> list[ApproachEfficacy]:
        ...
        rows = conn.execute(sql, params).fetchall()
        return [ApproachEfficacy(problem_type=r["problem_type"], approach=r["approach"], total_attempts=r["total_attempts"], successes=r["successes"], avg_duration_seconds=r["avg_duration_seconds"], avg_tokens=r["avg_tokens"], best_model=r["best_model"]) for r in rows]
    ...
    def learn_pattern(self, pattern_type: str, description: str, examples: list[str], source_session_id: str, confidence: float = 1.0) -> CodePattern:
        ...
        if existing:
            ...
            conn.execute("INSERT INTO patterns VALUES (?, ?, ?, ?, ?)", (pid, pattern_type, description, confidence, self._now()))
        ...
        conn.commit()
        return CodePattern(id=pid, pattern_type=pattern_type, description=description, examples=examples, confidence=existing["confidence"] if existing else confidence, source_sessions=[source_session_id])

    def get_patterns_by_type(self, pattern_type: str) -> list[CodePattern]:
        conn = self._get_conn()
        rows = conn.execute("SELECT * FROM patterns WHERE pattern_type = ? ORDER BY confidence DESC", (pattern_type,)).fetchall()
        result: list[CodePattern] = []
        for r in rows:
            exs = conn.execute("SELECT file_path FROM pattern_examples WHERE pattern_id = ?", (r["id"],)).fetchall()
